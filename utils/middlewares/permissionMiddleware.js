...
exports.getUserPermission = (req, res, next) => {
  var decoded = jwt.verify(req.body.access_token, project_config.secret_access_token_key); //декодим токен
  ...
  for (let i = 0; i< decoded.permissions.length; i++) {                                 // Проходим по правам пользователя
    if (permissions.indexOf(decoded.permissions[i].uuid !== -1))                        // Если право есть в конфиге
